...
const typesRdvBase = [
	...
	{
		id: "livraison" as TypeRdv,
		label: "Livraison",
		description: "Récupération de lunettes ou lentilles",
	},
];
...
export default function RendezVousPage() {
			...
			try {
				...
				if (data.configRdv?.durees) {
					setDurees({
						...
						livraison: data.configRdv.durees.livraison ?? DUREE_PAR_TYPE.livraison,
					});
				}
			} catch {  }
		}
	...
	const estFerme = useCallback(
		(iso: string): boolean => {
			if (ouverturesSet.has(iso)) return false;
			const dayOfWeek = new Date(iso + "T12:00:00").getDay();
			const h = horaires.find((hr) => hr.jour === dayOfWeek);
			if (h && !h.ouvert) return true;
			for (const v of vacances) {
				if (iso >= v.dateDebut.slice(0, 10) && iso <= v.dateFin.slice(0, 10)) return true;
			}
			return fermeturesJournee.has(iso);
		},
		[ouverturesSet, horaires, vacances, fermeturesJournee]
	);

	const moisCleIso = (iso: string) => {
		const [a, m] = iso.split("-").map(Number);
		return a * 12 + (m - 1);
	};
	const moisAffiche = viewMonth.getFullYear() * 12 + viewMonth.getMonth();
	const peutReculer = moisAffiche > moisCleIso(firstDate);
	const casesMois = useMemo(() => {
		const annee = viewMonth.getFullYear();
		const mois = viewMonth.getMonth();
	...
	const handleSelectDate = (date: string) => {
		...
		if (typeRdv) fetchCreneaux(date, typeRdv);
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (formData.telephone) {
			const digits = formData.telephone.replace(/\s/g, "");
			if (!/^0[1-9]\d{8}$/.test(digits)) {
				setError("Numéro de téléphone invalide. Veuillez saisir un numéro français à 10 chiffres (ex : 06 12 34 56 78).");
				return;
			}
		}
				...
				<div className="max-w-3xl mx-auto">
					...
					{step === 2 && (
						...
						>
							...
							<div className="flex flex-col sm:flex-row justify-center items-center sm:items-stretch overflow-x-clip sm:overflow-visible">
								<motion.div
									ref={calendarRef}
									layout
									transition={{ type: "spring", stiffness: 260, damping: 30 }}
									className="relative z-20 w-full max-w-90 rounded-2xl bg-white p-5 shadow-lg"
								>
									<div className="flex items-center justify-between mb-4">
										<button
											...
											className="w-8 h-8 flex items-center justify-center rounded-full text-brown hover:bg-brown/5 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-colors"
										>
											<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
												<path d="M15 18l-6-6 6-6" />
											</svg>
										</button>
										<p className="font-medium text-brown capitalize">
									...
									<div className="grid grid-cols-7 gap-1 mb-1">
										{["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((j) => (
											<div key={j} className="text-center text-xs text-brown/40 font-medium py-1">
												{j}
											</div>
										))}
									</div>
									...
									<div className="grid grid-cols-7 gap-1">
										{casesMois.map((date, i) => {
											if (date === null) return <div key={`vide-${i}`} aria-hidden className="aspect-square" />;
											const indispo = date < firstDate || estFerme(date);
											return (
												<button
													key={date}
													type="button"
													onClick={() => !indispo && handleToggleDate(date)}
													disabled={indispo}
													className={`aspect-square flex items-center justify-center rounded-full text-sm transition-all duration-200 ${indispo
														? "text-brown/20 cursor-not-allowed"
														: selectedDate === date
															? "bg-accent text-white font-medium cursor-pointer"
															: "text-brown hover:bg-brown/10 cursor-pointer"
														}`}
												>
								...
								<AnimatePresence mode="popLayout">
									{selectedDate && (
										<motion.div
											...
											className="relative z-10 flex flex-col overflow-hidden w-full sm:w-80 shrink-0 mt-4 sm:mt-0 sm:-ml-6 rounded-2xl bg-white p-5 sm:pl-9 shadow-lg"
										>
											<p className="shrink-0 text-sm font-medium text-brown capitalize">
												{formatDateFr(selectedDate)}
											</p>
											<p className="shrink-0 text-xs text-brown/40 mb-4">Choisissez un horaire</p>
											<div className="flex-1 min-h-0 overflow-y-auto pr-1">
												{loadingSlots ? (
													...
													<div className="space-y-4">
														{(() => {
															const groupes = [
																{ label: "Matin", slots: creneaux.filter((h) => h < "13:00") },
																{ label: "Après-midi", slots: creneaux.filter((h) => h >= "13:00") },
															].filter((g) => g.slots.length > 0);
															return groupes.map((g) => (
																<div key={g.label}>
																	<p className="text-xs text-brown/40 uppercase tracking-wider mb-2">{g.label}</p>
																	<div className="grid grid-cols-3 gap-2">
																		{g.slots.map((heure) => (
																			<button
																				key={heure}
																				onClick={() => setSelectedHeure(heure)}
																				className={`py-2.5 px-2 rounded-xl border-2 text-sm font-medium transition-all duration-300 cursor-pointer ${selectedHeure === heure
													...
													<p className="text-sm text-brown/50 py-8 text-center">Aucun créneau disponible ce jour</p>
												)}
											</div>
										</motion.div>
									)}
