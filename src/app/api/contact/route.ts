...
async function handleLentilles(
  ...
  const {
    ...
    prenom: string;
    telephone: string;
    format: "journalieres" | "mensuelles" | "essai";
    duree: string;
  ...
  const estEssai = format === "essai";

  if (!nom || !prenom || !telephone || !format || (!estEssai && !duree) || !ordonnance) {
    return NextResponse.json(
  ...
  const formatLabel =
    format === "journalieres" ? "Journalières" : estEssai ? "À l'essai" : "Mensuelles";
  const dureeLabel = estEssai ? undefined : `${duree} mois`;

  const fullMessage = [
    "[Commande de lentilles]",
    `Format : ${formatLabel}`,
    dureeLabel ? `Durée souhaitée : ${dureeLabel}` : "",
    besoinProduit
